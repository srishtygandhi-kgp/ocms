...
def create_classroom(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            classroom = Classroom(
                name=name, description=description, time_slot=time_slot, created_by=request.user)
            classroom.save()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            topic = get_object_or_404(
                Topic, pk=int(form.cleaned_data['topics']))
...
def assignment_submit(request, pk):
    if request.method == 'POST':
        ...
        if not submitted_assignment:
            submitted_assignment = SubmittedAssignment.objects.create(
                assignment=assignment, user=request.user)
        files = request.FILES.getlist('file_field')
        print(files)
        for f in files:
            AssignmentFile.objects.create(
                submitted_assignment=submitted_assignment, files=f)

    form = AssignmentFileForm()
    private_comment_form = PrivateCommentForm()
    assignment = get_object_or_404(Assignment, pk=pk)
    submit_assignment = assignment.submittedassignment_set.filter(
        user=request.user)
    classroom_teachers = list(map(lambda teaches: teaches.teacher,
                              assignment.topic.classroom.classroomteachers_set.all()))
